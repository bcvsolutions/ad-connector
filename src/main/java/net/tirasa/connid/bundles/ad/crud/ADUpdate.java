    ...
    public Uid update(final Set<Attribute> attrs) {
        ...
        final ResultsHandler handler = new ResultsHandler() {
            ...
            public boolean handle(ConnectorObject connectorObject) {
                found.add(connectorObject);
                return false;
            }
        };

        final LdapFilter filter = getNativeUidFilter();
        new ADSearch(conn, oclass, filter, handler, new OperationOptions(new HashMap<String, Object>()))
            .executeADQuery(handler);

        if (found.isEmpty()) {
            throw new ConnectorException("Nothing found for filter " + filter);
        }

        final ConnectorObject obj = found.iterator().next();
        ...
        Uid newUid = uidChanged(attrsToBeUpdated);
        if (newUid != null) {
        	return newUid;
        }

        // Uid should not change ever, so we can return the one which was given to us
        return uid;
    }
    
    private Uid uidChanged(final Set<org.identityconnectors.framework.common.objects.Attribute> attrs) {
    	List<String> stringListValue = getStringListValue(attrs, conn.getConfiguration().getUidAttribute());
    	if (stringListValue != null && !stringListValue.isEmpty()) {
    		return new Uid(stringListValue.iterator().next());
    	}
    	return null;
    }

    private LdapFilter getNativeUidFilter() {
        StringBuilder filterBuilder = new StringBuilder();
        filterBuilder.append(conn.getSchemaMapping().getLdapUidAttribute(oclass))
            .append("=");
        if (!LdapUtil.escapeAttrValue(uid.getUidValue(), filterBuilder)) {
            throw new ConnectorException("Wrong UID attribute");
        }
        final String filter =  filterBuilder.toString();
        return LdapFilter.forNativeFilter(filter);
    }
