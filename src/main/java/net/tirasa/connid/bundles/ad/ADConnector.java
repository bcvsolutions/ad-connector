 ...
 */
@ConnectorClass(configurationClass = ADConfiguration.class, displayNameKey = "ad.connector.display")
public class ADConnector extends LdapConnector {
    ...
    public void executeQuery(
        ...
        OperationOptions ops = enhanceOperationOptions(options);
        //
        new ADSearch(conn, oclass, query, handler, ops).executeADQuery(handler);
    }

    private OperationOptions enhanceOperationOptions(OperationOptions options) {
        Map<String, Object> enhancedOptions = new HashMap<String, Object>(options.getOptions());
        enhancedOptions.put(OperationOptions.OP_PAGE_SIZE, config.getPageSize());

        // BCV - when IdM will support operations options, it will override connector options
        if(options.getAttributesToGet() == null) {
			enhancedOptions.put(OperationOptions.OP_ATTRIBUTES_TO_GET, config.getExtensionAttToGet());
		} else {
			Set<String> attrsOptions = new HashSet<String>(Arrays.asList(options.getAttributesToGet()));
			attrsOptions.addAll(Arrays.asList(config.getExtensionAttToGet()));
			
			enhancedOptions.put(OperationOptions.OP_ATTRIBUTES_TO_GET, attrsOptions.toArray());
		}
        return new OperationOptions(enhancedOptions);
    }
    ...
    public void sync(final ObjectClass oclass, final SyncToken token,
        ...
        OperationOptions ops = enhanceOperationOptions(options);
        syncStrategy.sync(token, handler, ops, oclass);
    }
